...
// Get bus by ID (with operator verification)
function getBusById($busId, $operatorId) {
    $db = getDatabaseConnection();
...
if (!$user) {
    header('Location: /login.php?redirect=operator/edit-bus.php');
    exit;
}
...
// Check if bus ID is provided and valid
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    die('Invalid bus ID');
}

$busId = (int)$_GET['id'];
$bus = getBusById($busId, $operator['operator_id']);
if (!$bus) {
    die('Bus not found or you do not have permission to edit it');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validate inputs (same as add-bus.php)
    if (empty($name)) {
    ...
    if (isset($_FILES['bus_image']) && $_FILES['bus_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($detectedType, $allowedTypes)) {
            ...
            if (move_uploaded_file($_FILES['bus_image']['tmp_name'], $destination)) {
                ...
                if (!empty($bus['image_url']) && strpos($bus['image_url'], '/uploads/buses/') === 0) {
                    @unlink('..' . $bus['image_url']);
                }
            } else {
    ...
    if (empty($errors)) {
        try {
            ...
            $imageToUse = !empty($uploadedImageUrl) ? $uploadedImageUrl : 
                         (!empty($imageUrl) ? $imageUrl : $bus['image_url']);
            ...
            // Refresh bus data after update
            $bus = getBusById($busId, $operator['operator_id']);
        } catch (PDOException $e) {
            $errors['database'] = 'Failed to update bus: ' . $e->getMessage();
        }
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Edit Bus: <?= htmlspecialchars($bus['name']) ?></title>
...
<body>
    <div class="container py-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h1 class="h3 mb-2 text-gray-800">Edit Bus: <?= htmlspecialchars($bus['name']) ?></h1>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="dashboard.php">Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="manage-bus.php">Buses</a></li>
        ...
        <?php if ($success): ?>
            <div class="alert alert-success d-flex align-items-center" role="alert">
                ...
                <div>
                    <h5 class="alert-heading mb-1">Bus updated successfully!</h5>
        ...
        <form method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
            ...
            <!-- Add a hidden field to track image removal -->
<input type="hidden" name="remove_image" id="removeImageFlag" value="0">
            ...
            <div class="row">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control <?= isset($errors['name']) ? 'is-invalid' : '' ?>" 
                                       id="name" name="name" value="<?= htmlspecialchars($_POST['name'] ?? $bus['name']) ?>" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control <?= isset($errors['model']) ? 'is-invalid' : '' ?>" 
                                       id="model" name="model" value="<?= htmlspecialchars($_POST['model'] ?? $bus['model']) ?>" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control <?= isset($errors['year']) ? 'is-invalid' : '' ?>" 
                                       ...
                                       value="<?= htmlspecialchars($_POST['year'] ?? $bus['year']) ?>" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control <?= isset($errors['registration_number']) ? 'is-invalid' : '' ?>" 
                                       ...
                                       value="<?= htmlspecialchars($_POST['registration_number'] ?? $bus['registration_number']) ?>" required>
                                ...
                                <select class="form-select <?= isset($errors['type_id']) ? 'is-invalid' : '' ?>" 
                                    ...
                                    <?php foreach ($busTypes as $type): ?>
                                        <option value="<?= $type['type_id'] ?>" 
                                            <?= ($_POST['type_id'] ?? $bus['type_id']) == $type['type_id'] ? 'selected' : '' ?>>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" class="form-control <?= isset($errors['seat_capacity']) ? 'is-invalid' : '' ?>" 
                                       ...
                                       value="<?= htmlspecialchars($_POST['seat_capacity'] ?? $bus['seat_capacity']) ?>" required>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="features" name="features" 
                                          rows="2"><?= htmlspecialchars($_POST['features'] ?? $bus['features']) ?></textarea>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="status" id="status_active" 
                                           value="active" <?= ($_POST['status'] ?? $bus['status']) === 'active' ? 'checked' : '' ?> required>
                                ...
                                <div class="form-check mt-2">
                                    <input class="form-check-input" type="radio" name="status" id="status_maintenance" 
                                           value="maintenance" <?= ($_POST['status'] ?? $bus['status']) === 'maintenance' ? 'checked' : '' ?>>
                                ...
                                <div class="form-check mt-2">
                                    <input class="form-check-input" type="radio" name="status" id="status_inactive" 
                                           value="inactive" <?= ($_POST['status'] ?? $bus['status']) === 'inactive' ? 'checked' : '' ?>>
                ...
                <div class="col-lg-4">
                    <div class="form-section">
                        ...
                        <?php if (!empty($bus['image_url'])): ?>
                            <div class="current-image-container">
                                <img src="<?= htmlspecialchars($bus['image_url']) ?>" class="current-image" alt="Current Bus Image">
                                <button type="button" class="btn btn-danger btn-sm remove-image-btn" id="removeImageBtn">
                                    <i class="bi bi-trash"></i>
                                </button>
                            </div>
                            <input type="hidden" id="original_image_url" name="original_image_url" value="<?= htmlspecialchars($bus['image_url']) ?>">
                        <?php endif; ?>
                        ...
                        <div class="mb-3">
                            <label for="bus_image" class="form-label">Upload New Image</label>
                    ...
                    <div class="d-grid gap-2">
                        ...
                        <a href="manage-buses.php" class="btn btn-outline-secondary">
    ...
    if (confirm('Are you sure you want to remove this image?')) {
        ...
        this.closest('.current-image-container').style.display = 'none';
    }
});
